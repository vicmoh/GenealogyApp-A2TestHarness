...
static bool compareIndividualsBool(const void* first,const void* second){
    if(first == second){
        printf("firt and second pointer are the same, return true\n");
        return true;
    }
    
    if(first == NULL || second == NULL)
    {
        printf("when comparing indi, it returned false\n");
        return false;
    }
    ...
    if(intresult == 0)
    {
        printf("boolResult = true\n");
        boolresult = true;
    }
    else
    {
        printf("boolREsult = false\n");
        boolresult = false;
    }
...
//    }
    ...
    while ((elem = nextElement(&iter)) != NULL){
        ...
        while ((elem2 = nextElement(&iter2)) != NULL){
            ...
            {
                printf("failed comparing the indi\n");
                return false;
            }
        ...
        {
            printf("failed, comparing indi size is differ\n");
            return false;
        }
...
static SubTestRec _tGetAnsSubTestAdv1(int testNum, int subTest){
    ...
    List anscestors = getAncestorListN(obj, individual, 2);
    printf("my anscestors size %d\n", getLength(anscestors));
    printf("my individual fam size %d\n", getLength(individual->families));
    printf("\n GLIST TEST\n%s\n", gListToJSON(anscestors));
